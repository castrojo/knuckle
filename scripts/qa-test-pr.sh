...
# Allocate VM name early to parallelize disk prep with CI (if needed)
...
# If we need a VM (Tier >= 1), start disk prep in background while CI runs
if [[ $TIER -ge 1 ]]; then
...
# Wait for background disk prep to complete before proceeding to VM setup
